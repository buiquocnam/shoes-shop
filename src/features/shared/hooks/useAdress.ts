...
export const useCreateAddress = (userId: string) => {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : "Có lỗi xảy ra khi thêm địa chỉ";
      toast.error(message);
    },
  });
};
...
export const useDeleteAddress = (userId: string) => {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : "Có lỗi xảy ra khi xóa địa chỉ";
      toast.error(message);
    },
  });
};
