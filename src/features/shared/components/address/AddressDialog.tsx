...
/**
 * Dialog component để thêm địa chỉ mới
 * Sử dụng cấu trúc giống AddressForm nhưng với style dialog
 */
export function AddressDialog({
    ...
    }, [open, userId, defaultIsDefault, form]);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-xl rounded-3xl p-0 overflow-hidden bg-white border-slate-200">
                <DialogHeader className="p-6 border-b border-slate-100 bg-slate-50">
                    <DialogTitle className="text-xl font-black uppercase ">
                        Thêm <span className="text-primary">Địa chỉ mới</span>
                    </DialogTitle>
                </DialogHeader>
                <form onSubmit={form.handleSubmit(onSubmit)} className="p-8 space-y-6">
                    ...
                    <Button
                        ...
                        className="w-full h-14 font-black uppercase rounded-2xl "
                    >
                        {createAddressMutation.isPending ? "Đang thêm..." : "Lưu địa chỉ"}
                    </Button>
                </form>
            </DialogContent>
        </Dialog>
